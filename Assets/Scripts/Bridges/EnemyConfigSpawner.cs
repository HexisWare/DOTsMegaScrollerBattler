...
public class EnemyConfigSpawner : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        if (enemyConfigJson == null) { Debug.LogError("[EnemyConfigSpawner] No enemyConfigJson assigned.", this); enabled = false; return; }
        _cfg = SpawnConfigLoader.LoadFromText(enemyConfigJson.text);
        if (_cfg == null || _cfg.boxes == null || _cfg.boxes.Length == 0) { Debug.LogError("[EnemyConfigSpawner] Parsed config is empty/invalid.", this); enabled = false; return; }
        ...
        if (_box == null || _box.spawns == null || _box.spawns.Length == 0) { Debug.LogError($"[EnemyConfigSpawner] Box '{wantedId}' invalid.", this); enabled = false; return; }

        _spawn = System.Array.Find(_box.spawns, s => s.enabled);
        if (_spawn == null) { Debug.LogWarning($"[EnemyConfigSpawner] Box '{wantedId}' has no ENABLED spawns.", this); enabled = false; return; }

        _spawnInterval = (_spawn.spawnCooldown > 0f) ? _spawn.spawnCooldown : fallbackInterval;
        _timer = 0f;

        Debug.Log($"[EnemyConfigSpawner] Lane={lane} Box='{_box.id}' Spawn='{_spawn.id}'  spawnCooldown={_spawn.spawnCooldown:F2}s  intervalUsed={_spawnInterval:F2}s  attackCooldown={_spawn.cooldown:F2}s", this);
    }

    void Update()
    {
        // 👇 fully stop if building is dead
